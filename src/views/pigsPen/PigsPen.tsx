...
function PigsPen() {
	...
	const pigsBusdPrice = useAppSelector(
		(state) => state.pricingReducer.pigsBusdPrice
	);
	/// State for input
	...
	const shouldDisableMainButton = (): boolean => {
		...
		if (
			new BigNumber(userData.allowance).isLessThan(getDecimalAmount(inputValue))
		) {
			return true;
		}
		return false;
	...
	const checkButtonAndApproval = (inputvalue: string) => {
		...
		if (
			new BigNumber(userData.allowance).isGreaterThanOrEqualTo(
	...
	const depositPigs = async () => {
		...
		try {
			...
			if (
				new BigNumber(userData.earningsBusd).isGreaterThan(0) ||
				new BigNumber(userData.earningsPigs).isGreaterThan(0)
			) {
	...
	const isWithdrawalEnabled =
		Math.floor(Date.now() / 1000) - Number(userData.startLockTimestamp) > 86400;
	...
	const modalDetailsForWithdraw = {
		...
		value: new BigNumber(
			getBalanceAmountString(userData.pigAvailableForWithdrawal)
		).toFormat(2),
		text: "PIGS",
		...
		infoValues: [
			{
				...
				value: `${amountFormatter(
					getBalanceAmountString(
						(
							Number(userData.stakedBalance) -
							Number(userData.pigAvailableForWithdrawal)
						).toString()
					)
				)}`,
			},
		...
		<animated.div style={props} className={styles.pigspen__wrap}>
			<div className={styles.pigspen}>
				...
				<div className={styles.cards}>
					<div>
						<PigsCreditCard
							title="Total PIGS Locked"
							amount={`${new BigNumber(
								getBalanceAmountString(pigPenData.pigsSupply)
							).toFormat(2)} PIGS`}
						/>
					</div>
					<div>
						<PigsCreditCard
							...
							amount={`$ ${new BigNumber(
								getBalanceAmountString(
									(
										Number(pigPenData.pigsSupply) * Number(pigsBusdPrice) +
										Number(pigPenData.busdRewards) +
										Number(pigPenData.pigsRewards) * Number(pigsBusdPrice)
									).toString()
								)
							).toFormat(2)}`}
						/>
					...
					<div>
						<PigsCreditCard
							title="BUSD in Rewards Vault"
							amount={`$ ${new BigNumber(
								getBalanceAmountString(pigPenData.busdRewards)
							).toFormat(2)} BUSD`}
						/>
					</div>
					<div>
						<PigsCreditCard
							title="PIGS in Rewards Vault"
							amount={`${new BigNumber(
								getBalanceAmountString(pigPenData.pigsRewards)
							).toFormat(2)} PIGS`}
						/>
					</div>
				</div>
					...
					<div className={styles.credit__wrap__in}>
						...
						<div className={styles.tabs}>
							<div
								...
								className={
									activeTab === 1
										? `${styles.tab__one} ${styles.tab__one__active}`
										: `${styles.tab__one}`
								}
							>
								<p>Deposit PIGS</p>
							...
							<div
								...
								className={
									activeTab === 2
										? `${styles.tab__two} ${styles.tab__two__active}`
										: `${styles.tab__two}`
								}
							>
								<p>Withdraw PIGS</p>
						...
						{activeTab === 1 ? (
							<RewardsCenter
								...
								infoValue={`${new BigNumber(
									getBalanceAmountString(userData.stakedBalance)
								).toFormat(2)} PIGS`}
