...
// APIs
import {
	approveDripGardenForDripBusdLP,
	buySeeds,
	plantSeeds,
	sellSeeds,
	getEstimatedPlants,
...
function Garden() {
	...
	const usdValue = garden
		? new BigNumber(garden.userData.usdValue)
		: new BigNumber("0");
	const usdValueText = Number(usdValue) > 0 ? usdValue.toString() : "0";

	const marketEggs = garden
		? new BigNumber(garden.marketSeeds)
		: new BigNumber("0");
	const eggsToHatch = garden
		? new BigNumber(garden.seedsToHatch)
		: new BigNumber("0");

	const contractBalance = garden
		? new BigNumber(garden.balance)
		: new BigNumber("0");

	const userMiners = garden
		? new BigNumber(garden.userData.plants)
		: new BigNumber("0");
	const userMinersText = Number(userMiners) > 0 ? userMiners.toString() : "0";

	const userSecondsUntilFull = garden
		? new BigNumber(garden.userData.secondsUntilFull)
		: new BigNumber("0");

	const userSeeds = garden ? new BigNumber(garden.userData.seeds) : 0;
	const userSeedsText = Number(userSeeds) > 0 ? userSeeds.toString() : "0";

	const SeedProgress = garden
		? ((Number(userSeeds) / seedsToPlant) * 100).toString()
		: "0";
	const userSeedsAvailable = garden
		? new BigNumber(garden.userData.availableSeeds)
		: new BigNumber("0");
	const userAvailableText =
		Number(userSeedsAvailable) > 0 ? userSeedsAvailable.toString() : "0";
	const userPlantsAvailable = garden
		? Math.trunc(Number(garden.userData.seeds) / seedsToPlant)
		: "N/A";
	...
	const userClaimed = garden
		? new BigNumber(garden.userData.claimed)
		: new BigNumber("0");
	const userLast = garden
		? new BigNumber(garden.userData.last)
		: new BigNumber("0");
	const userReferrals = garden
		? new BigNumber(garden.userData.referrals)
		: new BigNumber("0");
	const productionRate = garden ? userMiners : 0;
	const productionRateDay =
		productionRate > 0 ? userMiners.multipliedBy(60 * 60 * 24) : "0";

	// const timeTillGrown = Math.trunc((seedsToPlant - Number(userSeeds) / Number(productionRate)))
	const timeTillGrown =
		(seedsToPlant - Number(userSeeds)) / Number(productionRate);
	const userTimeRemainingText =
		productionRate > 0 ? secondsToString(timeTillGrown) : "N/A";

	const timeMultiplier = garden
		? (Number(garden.timeMultiplier) / 1e7).toFixed(2)
		: "N/A";
	...
	const balanceMultiplier = garden
		? (Number(garden.balanceMultiplier) / 1e4).toFixed(2)
		: "N/A";
	const balanceMultiplierText = "294.29";
	const totalMultiplierText = garden
		? `${(Number(timeMultiplier) + Number(balanceMultiplierText)).toFixed(2)}%`
		: "N/A";
	...
	useEffect(() => {
		if (params.referee) {
			...
			setReferralAddress(params.referee)
		}
	...
	useEffect(() => {
		...
		if (
			new BigNumber(garden.userData.lpBalance).isLessThan(
		...
		if (
			new BigNumber(garden.userData.lpAllowance).isLessThan(
				getDecimalAmount(inputValue)
			)
		) {
			setShowApprove(true);
	...
	const estimatePlants = async (value: string) => {
		const amountOfPlants = await getEstimatedPlants(
			getDecimalAmount(amountFormatter(value)),
			signer
		);
	...
	const copyRefLink = () => {
		if (navigator.clipboard && navigator.permissions) {
			navigator.clipboard
				.writeText(`${window.location.origin}/garden/${account}`)
				.then(() => {
					// ..
		...
		<div className={styles.addliquidity__wrap}>
			<div className={styles.addliquidity}>
				<div className={styles.cards}>
					<PigsCreditCard
						title="Total LP locked"
						amount={`${garden.balance} DRIP/BUSD`}
					/>
				</div>
				...
				<section>
					...
					<p className={styles.info}>
						...
						<a
							href="https://theanimal.farm/dripliberation"
							target="_blank"
							rel="noreferrer"
						>
							DRIP Liberation
						...
						<div className={styles.balance}>
							<p
								onClick={() => {
									setInputValue(
										amountFormatter(
											getBalanceAmountString(garden.userData.lpBalance)
										)
									);
								}}
								role="presentation"
					...
					<div className={styles.reward__center}>
						...
						<div className={styles.info__area}>
							...
							<Info
								title="Seeds Available Value"
								info={`$${usdValueText.toString()} (${userAvailableText} LP)`}
							/>
							<Info
								title="Seeds Per Day"
								info={`${productionRateDay.toString()}`}
							/>
						</div>
						<div className={styles.progress}>
							<p>
								{userTimeRemainingText} until ready to sow (
								{userPlantsAvailable} plants)
							</p>
				...
				<div className={styles.about}>
					...
					<p>
						The DRIP Garden is a true game where all players start out
						relatively equal and what determines your payout is how often you
						plant your seeds(compound) vs. how often you sell them (withdraw).
						Below is a graph showing the buying power over time and balance
						compared to other games such as Shrimp Farmer and
						&quot;Miners&quot;. Existing games give insiders over 100,000% more
						buying power than players who get in after launch, allowing them to
						drain the contract and disincentives new capital from coming in.
					</p>
					<h3>Drip Garden Mechanism</h3>
					<img src={gardenGraphImage} alt="" />
					<p>
						As you can see, the DRIP Garden increases buying power over time and
						as the balance of the contract grows ensuring that you will always
						get a fair rate. There is still a benefit to getting in early
						because the earlier your plants get in the ground the larger you can
						grow your garden and out perform the competition.{" "}
					</p>

					<p>
						The time multiplier is designed to attract new waves capital after
						we have reached saturation and the contract growth has slowed due to
						the buying power of new capital creeping up by 0.1% non compounding
						every day. This means there will always be a time in the future
						where it makes sense for fresh capital to come in and kick start a
						new wave gardeners!
					</p>
				</div>
			</div>
